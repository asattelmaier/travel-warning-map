      ...
      .then(data => {
        this.geoJsonLayer = L.geoJSON(data, {
          ...
          onEachFeature: (feature, layer) => {
            ...
            // Initial click listener regardless of current warning state
            // Logic is deferred to click time to ensure we use the freshest data
            layer.on('click', () => {
              // Lookup fresh warning data
              const currentWarning = this.travelWarnings.find(w => w.countryCode === countryCode);

              if (!currentWarning) {
                // No warning data for this country, do nothing or show info
                return;
              }

              // Proceed with showing details for currentWarning
              this.travelWarningService.getWarningDetails(currentWarning.id).subscribe({
                next: (details) => {
                  this.modalTitle = currentWarning.countryName;
                  this.modalStatus = currentWarning.warning ? 'Reisewarnung' : currentWarning.partialWarning ? 'Teilweise Reisewarnung' : 'Keine Reisewarnung';
                  this.modalLastModified = new Date(details.response[currentWarning.id].lastModified * 1000).toLocaleDateString('de-DE');
                  this.modalEffective = new Date(details.response[currentWarning.id].effective * 1000).toLocaleDateString('de-DE');
                  this.modalContent = details.response[currentWarning.id].content || 'Keine Beschreibung verfügbar';
                    ...
                    const popupContent = `
                      <div class="min-w-[300px] font-sans text-slate-800">
                        ...
                        <div class="bg-brand-offwhite px-5 py-3 border-b border-ui-border flex items-center justify-between">
                           <h3 class="text-lg font-bold text-text-primary">${currentWarning.countryName}</h3>
                           <div class="${currentWarning.warning ? 'bg-brand-red' : currentWarning.partialWarning ? 'bg-brand-yellow' : 'bg-brand-green'} text-white rounded-full p-1.5">
                        ...
                        <div class="p-5 bg-surface">
                          <div class="space-y-3 mb-5">
                            <div class="flex items-start justify-between">
                              ...
                              <span class="font-bold ${currentWarning.warning ? 'text-brand-red' : currentWarning.partialWarning ? 'text-brand-yellow' : 'text-brand-green'}">${currentWarning.warning ? 'Reisewarnung' : currentWarning.partialWarning ? 'Teilweise Reisewarnung' : 'Keine Reisewarnung'}</span>
                            </div>
                            <div class="flex items-start justify-between">
                              <span class="text-sm text-text-secondary font-medium">Aktualisiert</span>
                              <span class="font-semibold text-text-primary">${new Date(details.response[currentWarning.id].lastModified * 1000).toLocaleDateString('de-DE')}</span>
                            </div>
                            <div class="flex items-start justify-between">
                              <span class="text-sm text-text-secondary font-medium">Gültig seit</span>
                              <span class="font-semibold text-text-primary">${new Date(details.response[currentWarning.id].effective * 1000).toLocaleDateString('de-DE')}</span>
                            </div>
                          </div>
